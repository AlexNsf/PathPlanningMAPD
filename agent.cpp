...
bool Agent::update_path(const Map& map, const Task& task, Token& token) {
    ...
    for (int i = 0; i < path_to_finish.size(); ++i) {
        if (i == 0) {
            path_to_finish[i].prev_i = path_to_start[path_to_start.size() - 1].i;
            path_to_finish[i].prev_j = path_to_start[path_to_start.size() - 1].j;
        } else {
    ...
    finish_time = start_time + path_to_start.size() + path_to_finish.size();
    return true;
}
